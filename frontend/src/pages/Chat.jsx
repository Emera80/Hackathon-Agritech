...
import { Sprout, Plus, MessageSquare, Wheat, Search, LogOut } from 'lucide-react';
...
function Chat() {
  ...
  const fetchSessions = async () => {
    try {
      ...
      if (err.response?.status === 401) {
        ...
        window.location.href = '/login'; // Déconnexion forcée si token invalide
      }
    }
  };
  ...
  const loadSession = async (id) => {
    ...
    try {
      ...
      setIsSidebarOpen(false); // Fermer le menu sur mobile après chargement
    } catch (err) {
      if (err.response?.status === 401) {
        localStorage.removeItem('token');
  ...
  const handleSendMessage = async (text, file = null) => {
    ...
    if (file) {
      ...
      if (isVideo) {
        userMessage.videoPreview = URL.createObjectURL(file);
      } else {
    ...
    setMessages(prev => [...prev,
      ...
      { id: Date.now() + 1, sender: 'bot', text: '', isStreaming: true }
    ]);
    setIsLoading(true);
    ...
    if (abortControllerRef.current) {
      ...
      setMessages(prev => {
        ...
        if (last?.sender === 'bot' && last.isStreaming) {
          u[u.length - 1] = { ...last, isStreaming: false, text: last.text + " [Génération interrompue]" };
        }
        return u;
      });
    }
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: '#f8f7f4' }}>
      ...
      {/* ─── SIDEBAR ─────────────────────────────────────────────────────────── */}
      <aside
        className={`fixed inset-y-0 left-0 z-[70] md:relative md:z-0 flex flex-col flex-shrink-0 h-full transition-all duration-300 ${
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-5 border-b border-white/5">
          ...
          {!sidebarCollapsed && (
            <div className="overflow-hidden flex-1">
              <p className="text-white font-bold text-sm leading-none">AgriBot AI</p>
          ...
          <button
            ...
            className="hidden md:flex text-white/30 hover:text-white/70 transition-colors flex-shrink-0"
          >
        ...
        <div className="px-3 py-3 flex flex-col gap-2">
          <button
            ...
            className="flex items-center gap-2 w-full px-3 py-2 rounded-xl text-red-400 font-medium text-sm transition-all hover:bg-red-500/10 border border-transparent hover:border-red-500/20"
          >
      ...
      <div className="flex flex-col flex-1 min-w-0 h-full">
        ...
        {/* Header avec bouton session mobile */}
        <header className="flex items-center justify-between px-4 md:px-6 py-3 border-b border-black/5 bg-white/60 backdrop-blur-sm flex-shrink-0">
          <div className="flex items-center gap-3">
             {/* Menu Hamburger Mobile */}
             <button
               onClick={() => setIsSidebarOpen(true)}
               className="md:hidden p-2 -ml-2 rounded-lg hover:bg-black/5 text-gray-500"
             >
               <MessageSquare size={20} />
             </button>

             <button
               onClick={() => { loadSession(null); setIsWelcome(true); }}
               className="md:hidden p-1.5 rounded-lg hover:bg-black/5 text-green-600"
               title="Nouvelle discussion"
             >
               <Plus size={20} />
             </button>
             <p className="text-sm font-semibold text-gray-600 truncate max-w-[150px] sm:max-w-[250px]">
               {currentSessionId
        ...
        <main className="flex-1 overflow-y-auto chat-scrollbar">
          {isWelcome ? (
            /* ─── ÉTAT D'ACCUEIL ─── */
            <div className="flex flex-col items-center justify-start md:justify-center min-h-full px-4 md:px-6 py-10 md:py-12 gap-8 md:gap-10 max-w-2xl md:max-w-5xl mx-auto">
              <div className="scale-90 md:scale-110 mb-2">
                <Orb isSpeaking={isTTSSpeaking} isLoading={isLoading} />
              </div>
              <div className="text-center animate-[hero-fade-up_0.8s_ease-out] w-full px-4">
                <h2 className="text-3xl md:text-5xl font-extrabold mb-4 tracking-tight welcome-gradient-text">
                  Aslama, Agriculteur !
                </h2>
              ...
              {/* Suggestions rapides */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full px-4 max-w-3xl mx-auto">
                {SUGGESTIONS.map((s, i) => (
                  <button
                    key={i}
            ...
            /* ─── MESSAGES ─── */
            <div className="w-full max-w-lg md:max-w-4xl lg:max-w-5xl mx-auto px-4 md:px-6 py-6 space-y-4">
              ...
              {isLoading && (
                <div className="flex gap-3 items-start">
                  ...
                  <div className="bg-white rounded-2xl rounded-tl-none px-4 py-3 shadow-sm border border-gray-100 flex flex-col gap-2">
                    <AgriLoader />
